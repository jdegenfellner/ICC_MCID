...
# https://www.digotor.info/Downloads/Scores/WS/Roland_and_Morris_Disability_Questionnaire_RMDQ_09-2023.pdf?m=1696626875&
# "Die minimale klinisch relevante Veränderung wird von diversen Autoren
# unterschiedlich angegeben und beträgt zwischen 2-8 Punkten. Es wird daher vorgeschlagen
# diese Grenze abhängig vom Behinderungsgrad zu bestimmen"
...
# How often is x_TP2 within the MCIC of 2 points?---------
df$abs_diff <- abs(df$x_TP1 - df$x_TP2)
hist(df$abs_diff, breaks = 20)
abline(v = 2, col = "red")

table(df$abs_diff > 2)/sum(table(df$abs_diff > 2)) # 
# -> in ~40% of cases we detect a minimally clinically important change of 2 points.
# even though the underlying truth did not change.
# for an ICC of 0.84, this number would be above 50%, i.e. we could guess.
